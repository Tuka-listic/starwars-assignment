...
const CardDetails = () => {
    ...
    useEffect (() => {
        setLoading (true);
        const getData = async() => {
            const reponse = await axios.get(`https://swapi.dev/api/films${pathname}`)
        const character = await reponse.data.characters.map((url) => axios.get(url).then((res) => res.data.name)) 
        const planet = await reponse.data.planets.map((url) => axios.get(url).then((res) => res.data.name)) 
        const specie = await reponse.data.species.map((url) => axios.get(url).then((res) => res.data.name)) 
        const starShip = await reponse.data.starShips.map((url) => axios.get(url).then((res) => res.data.name)) 
        const vehicle = await reponse.data.vehicles.map((url) => axios.get(url).then((res) => res.data.name)) 
 
        setData(await reponse.data);
        setCharacters(await Promise.all(character));
    ...
    <section>
        ...
        <div className='characters'>
            <h3>Characters</h3>
            <ul>
                {characters.map((character, index) => (<li key={index}>{(character)}</li>))}
            </ul>
        </div>
        <div className='characters'>
            <h3>Planets</h3>
            <ul>
                {planets.map((character, index) => (<li key={index}>{(character)}</li>))}
            </ul>
        </div>
        <div className='characters'>
            <h3>Species</h3>
            <ul>
                {species.map((character, index) => (<li key={index}>{(character)}</li>))}
            </ul>
        </div>
        <div className='characters'>
            <h3>Starships</h3>
            <ul>
                {starShips.map((character, index) => (<li key={index}>{(character)}</li>))}
            </ul>
        </div>
        <div className='characters'>
            <h3>Vehicles</h3>
            <ul>
                {vehicles.map((character, index) => (<li key={index}>{(character)}</li>))}
            </ul>
        </div>
    </section>
    </>
